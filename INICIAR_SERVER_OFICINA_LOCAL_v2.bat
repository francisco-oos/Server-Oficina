...
REM Buscar automaticamente un puerto local disponible.
set "SERVER_OFICINA_PORT="
for %%P in (8090 8081 8000 8765 8888) do (
    if not defined SERVER_OFICINA_PORT (
        powershell -NoProfile -Command "$p=%%P; if(-not (Get-NetTCPConnection -State Listen -LocalPort $p -ErrorAction SilentlyContinue)){exit 0}else{exit 1}" >nul 2>&1
        if !errorlevel! EQU 0 set "SERVER_OFICINA_PORT=%%P"
    )
)

if not defined SERVER_OFICINA_PORT (
    echo [ERROR] No se encontro un puerto libre entre 8090, 8081, 8000, 8765 y 8888.
    echo Cierra algun servicio local o edita este archivo para agregar otro puerto.
    pause
    exit /b 1
)
...
echo       http://127.0.0.1:%SERVER_OFICINA_PORT%
echo.
echo Puerto 8080 se evita si esta ocupado por PostgreSQL/EDB u otro servicio.
echo Para detener el servidor presiona CTRL+C.
echo ============================================================
echo.

start "" cmd /c "timeout /t 2 /nobreak >nul & start http://127.0.0.1:%SERVER_OFICINA_PORT%"
